...
library(readr)
library(data.table)
cohort_p <- read.csv("/home/unix/sliang/sliang/age/cohort_p_sex_specific.csv")
cohort_p<-merge(cohort_p,UKB_PRS_58,by="id",all.x=TRUE)

cohort_p_m<-cohort_p[(cohort_p$Sex == "Male"),]
cohort_p_f<-cohort_p[(cohort_p$Sex == "Female"),]

fwrite(cohort_p_m,"cohort_p_m.csv")
fwrite(cohort_p_f,"cohort_p_f.csv")
