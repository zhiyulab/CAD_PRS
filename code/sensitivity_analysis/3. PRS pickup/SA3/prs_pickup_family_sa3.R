...
pheno<-pheno[,c("id","Sex","in_white_British_ancestry_subset")]
########################################################################################
...
family_cohort<-merge(family_cohort,pheno,by="id",all.x=TRUE)
family_cohort<-merge(family_cohort,UKB_PRS_58,by="id",all.x=TRUE)


family_cohort_white<-family_cohort[(family_cohort$in_white_British_ancestry_subset==1),]
family_cohort_other<-family_cohort[(family_cohort$in_white_British_ancestry_subset==0),]

family_cohort_m<-family_cohort[(family_cohort$Sex == "Male"),]
family_cohort_f<-family_cohort[(family_cohort$Sex == "Female"),]
...
results <- lapply(1:58, function(i) {
  col_name <- paste0("df", i, "_cutoff")
  x <- family_cohort_white[[col_name]]
  x <- x[!is.na(x)]
...
results <- lapply(1:58, function(i) {
  col_name <- paste0("df", i, "_cutoff")
  x <- family_cohort_other[[col_name]]
  x <- x[!is.na(x)]
