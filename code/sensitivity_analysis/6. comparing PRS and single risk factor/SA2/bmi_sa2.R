...
colnames(bmi)
library(readr)
family_p <- read.csv("/home/unix/sliang/sliang/family/family_p.csv")

#take top 10% cutoff as before with PRS
cutoff <- quantile(bmi$BMI, 0.9, na.rm = TRUE)
bmi$cutoff_10 <- ifelse(bmi$BMI >= cutoff, 1, 0)
family_p_BMI<-merge(family_p,bmi,by="id",all.x=TRUE)

###################################################
library(boot)

# Select the column
x <- family_p_BMI$cutoff_10
...
family_p_lpa_crp<-merge(family_p,lpa_crp,by="id",all.x=TRUE)
