...
cohort_p_55<-cohort_p_55[,1]

#take top 10% cutoff as before with PRS
cutoff <- quantile(bmi$BMI, 0.9, na.rm = TRUE)
bmi$cutoff_10 <- ifelse(bmi$BMI >= cutoff, 1, 0)
cohort_p_55_BMI<-merge(cohort_p_55,bmi,by="id",all.x=TRUE)
print(sum(cohort_p_55_BMI$cutoff_10==1,na.rm=T))
print(sum(is.na(cohort_p_55_BMI$cutoff_10)))
fwrite(cohort_p_55_BMI,"cohort_p_55_BMI.csv")
...
cohort_p_55_lpa_crp<-merge(cohort_p_55,lpa_crp,by="id",all.x=TRUE)
print(sum(cohort_p_55_lpa_crp$lpa_cutoff==1,na.rm=T))
print(sum(is.na(cohort_p_55_lpa_crp$lpa_cutoff)))
print(sum(cohort_p_55_lpa_crp$crp_cutoff==1,na.rm=T))
print(sum(is.na(cohort_p_55_lpa_crp$crp_cutoff)))
fwrite(cohort_p_55_lpa_crp,"cohort_p_55_lpa_crp.csv")
...
png("myplot_p.png", width = 6, height = 4, units = "in", res = 300)
