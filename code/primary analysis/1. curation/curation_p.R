...
cohort_p_strict<-subset(cohort_p_55,cohort_p_55$PCE<0.02)

fwrite(cohort_p_all, file.path(output_dir, "cohort_p_all.csv"))
fwrite(cohort_p_55, file.path(output_dir, "cohort_p_55.csv"))
fwrite(cohort_p_strict, file.path(output_dir, "cohort_p_strict.csv"))

complete_data_intermediate<-subset_data_intermediate[subset_data_intermediate$id %in% data_intermediate$id, ]
not_cohort_p <- complete_data_intermediate[!complete_data_intermediate$id %in% cohort_p_all$id, ]
fwrite(not_cohort_p,file.path(output_dir, "not_cohort_p.csv"))

mean(cohort_p_all$PCE)
sd(cohort_p_all$PCE, na.rm=T)
t.test(cohort_p_all$PCE,cohort_p_55$PCE)