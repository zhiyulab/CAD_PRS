...
summary_df_55_final <- summary_df_55_final %>%
  mutate(
    Group = factor(
      Group,
      levels = c("Total", "Subset_White", "Subset_Other")
    )
  ) %>%
  mutate(
    Group = case_when(
      Group == "Total" ~ "All Ancestries",
      Group == "Subset_White" ~ "Non-Hispanic White Ancestry",
      Group == "Subset_Other" ~ "Other Ancestries",
...
ggplot(summary_df_55_final,
  ...
  scale_color_manual(values = c(
    "Non-Hispanic White Ancestry" = "steelblue",
    "Other Ancestries" = "orange",
    "All Ancestries" = "red"
  )) +
...
summary_df_55_final<-logistic_results
colnames(summary_df_55_final) <-
...
plot_one_group <- function(data, group_name, color,
  ...
  ggplot(data, aes(x = PRS, y = HR)) +
    ...
    labs(
      title = group_name,
      y = if (show_y_label) "HR/SD" else NULL
    )
}
