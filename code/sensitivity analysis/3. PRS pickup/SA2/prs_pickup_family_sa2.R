...
library(readr)
library(data.table)
family_cohort <- read.csv("/home/unix/sliang/sliang/family/family_history_both_CAD.csv")
names(family_cohort)[1] <- "id"

cohort_p<-read.csv("/home/unix/sliang/sliang/family/cohort_p_55.csv")
family_p<-cohort_p[cohort_p$id %in% family_cohort$id,]
family_p<-merge(family_p,pheno,by="id",all.x=TRUE)
fwrite(family_p,"family_p.csv")
family_p<-merge(family_p,UKB_PRS_58,by="id",all.x=TRUE)

family_p_other<-family_p[(family_p$in_white_British_ancestry_subset==0),]
family_p_white<-family_p[(family_p$in_white_British_ancestry_subset==1),]
family_p_m<-family_p[(family_p$Sex == "Male"),]
family_p_f<-family_p[(family_p$Sex == "Female"),]

fwrite(family_p_other,"family_p_other.csv")
