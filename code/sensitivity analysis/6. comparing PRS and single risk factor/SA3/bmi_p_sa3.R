...
family_cohort_BMI<-merge(family_cohort,bmi,by="id",all.x=TRUE)

###################################################
library(boot)

# Select the column
x <- family_cohort_BMI$cutoff_10
...
family_cohort_lpa_crp<-merge(family_cohort,lpa_crp,by="id",all.x=TRUE)
...
# Select the column
x <- family_cohort_lpa_crp$lpa_cutoff
x <- x[!is.na(x)]  # remove NAs
...
# Select the column
x <- family_cohort_lpa_crp$crp_cutoff
x <- x[!is.na(x)]  # remove NAs
